...
function CartPage() {
  ...
  const handleDecrease = (item) => {
    if (item.quantity > 1) {
      fetch(`http://localhost:8080/cart/${item._id}`, {
        method: "PUT",
        headers: {
      ...
      <div className='bg-gradient-to-r from-[#FAFAFA] fron-0% to-[#FCFCFC] to-100%'>
        <div className='py-36 flex flex-col items-center justify-center gap-8'>
          ...
          <div className="space-y-7 px-4">
            <h3 className="md:text-5xl text-4xl md:leading-nug leading-snug font-bold">Item Add To The <span className="text-green">Cart</span></h3>
          </div>
        </div>
      </div>
      {/** table */}
