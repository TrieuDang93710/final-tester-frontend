...
function NavBar() {
  ...
  return (
    <header className='max-w-screen-2xl container mx-auto bg-white fixed top-0 left-0 right-0 transition-all duration-300 ease-in-out'>
      <div
        className={`navbar xl:px-24 ${sticky ? 'shadow-md  text-black transition-all duration-300 ease-in-out' : ''}`}
      >
        ...
        <div className='navbar-end'>
          ...
          {!isAdmin && user !== null && (
            <Link to={'/cart-page'}>
              <div
                tabIndex={0}
                role='button'
                className='btn btn-ghost btn-circle lg:flex hidden items-center justify-center mr-3'
              >
          ...
          {user ? (
            ...
            <button
              ...
              className='btn rounded-full bg-green px-6 flex items-center gap-2 text-white'
            >
              <FaRegUser /> Login
            </button>
          )}
